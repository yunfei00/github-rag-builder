...
owner: chroma-core
repo: chroma
path: docs/mintlify/cloud/search-api/search-basics.mdx
url: https://github.com/chroma-core/chroma/blob/main/docs/mintlify/cloud/search-api/search-basics.mdx
---
---
title: Search Basics
description: Learn how to construct and use the Search class for querying your Chroma collections.
---

import { Callout } from '/snippets/callout.mdx';

This page covers the basics of Search construction. For detailed usage of specific components, see:
- Filtering with Where - Complex filter expressions with `K()` and `.where()`
- Ranking and Scoring - Using `Knn` and `.rank()` for vector search
- Pagination and Selection - Field selection with `.select()` and pagination with `.limit()`
...
- `.where()` - See Filter expressions
- `.rank()` - See Ranking and scoring
- `.limit()` - See Pagination
- `.select()` and `.select_all()` - See Field selection
