...
owner: run-llama
repo: llama_index
path: docs/src/content/docs/framework/getting_started/starter_example.mdx
url: https://github.com/run-llama/llama_index/blob/main/docs/src/content/docs/framework/getting_started/starter_example.mdx
---
---
sidebar:
  order: 5
title: Starter Tutorial (Using OpenAI)
---
...
LlamaIndex uses OpenAI's `gpt-3.5-turbo` by default, although this example uses `gpt-4o-mini` for the LLM. Make sure your API key is available to your code by setting it as an environment variable:
...
Now we can create a tool for searching through documents using LlamaIndex. With no `Settings.embed_model` configured, `VectorStoreIndex` falls back to `OpenAIEmbedding`, which will use the `text-embedding-ada-002` model by default. For new projects we recommend passing `OpenAIEmbedding(model="text-embedding-3-small")` explicitly.

  Prefer to run embeddings locally with no API key? Swap the hosted embedder
  for a local Sentence Transformers model in two lines:
...
See the local starter tutorial for a fully-local pipeline.
