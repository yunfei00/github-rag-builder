...
owner: run-llama
repo: llama_index
path: docs/src/content/docs/framework/module_guides/querying/node_postprocessors/node_postprocessors.md
url: https://github.com/run-llama/llama_index/blob/main/docs/src/content/docs/framework/module_guides/querying/node_postprocessors/node_postprocessors.md
---
---
...
- No API key, local, recommended default: `SentenceTransformerRerank`, a cross-encoder via `sentence-transformers`. Use `cross-encoder/ms-marco-MiniLM-L6-v2` for speed, `Qwen/Qwen3-Reranker-0.6B` for stronger multilingual quality.
- Hosted API, minimal setup: `CohereRerank`, `JinaRerank`, `VoyageAIRerank`, `MixedbreadAIRerank`, `NVIDIARerank`.
- Highest quality, latency tolerant: `LLMRerank`, `RankGPTRerank`, `RankLLMRerank`. These use an LLM itself to judge relevance.
- Token-level late interaction: `ColbertRerank`, useful for narrow technical terminology.
- Multimodal (documents as images): `ColPaliRerank`.

See also the rerankers overview page for a broader framing.
...
- SentenceTransformer Rerank: cross-encoders via `sentence-transformers` (recommended default for local reranking).
- FlagEmbedding Reranker: `BAAI/bge-reranker-*` family.
- Colbert Rerank: token-level late interaction with ColBERT v2.
- OpenVINO Rerank: Intel-optimized cross-encoder inference.
- RankLLM: instruction-tuned listwise LLM rerankers (RankZephyr, RankVicuna).
...
- Structured LLM Reranker Lyft 10k
- RankGPT

Multimodal rerankers:

- ColPali Rerank: vision-language reranking over document images.
