...
owner: run-llama
repo: llama_index
path: docs/src/content/docs/framework/module_guides/indexing/vector_store_index.mdx
url: https://github.com/run-llama/llama_index/blob/main/docs/src/content/docs/framework/module_guides/indexing/vector_store_index.mdx
---
---
title: Using VectorStoreIndex
---
...
```
  ...
  If you are using `from_documents` on the command line, it can be convenient to
  pass `show_progress=True` to display a progress bar during index construction.
...
By default, VectorStoreIndex stores everything in memory. See Using Vector Stores below for more on how to use persistent vector stores.
  ...
  By default, the `VectorStoreIndex` will generate and insert vectors in batches
  of 2048 nodes. If you are memory constrained (or have a surplus of memory),
  you can modify this by passing `insert_batch_size=2048` with your desired
  batch size.
...
```
  ...
  You can learn more about how to use the ingestion
  pipeline.
