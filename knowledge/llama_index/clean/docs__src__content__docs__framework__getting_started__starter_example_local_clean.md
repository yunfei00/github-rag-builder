...
owner: run-llama
repo: llama_index
path: docs/src/content/docs/framework/getting_started/starter_example_local.mdx
url: https://github.com/run-llama/llama_index/blob/main/docs/src/content/docs/framework/getting_started/starter_example_local.mdx
---
---
sidebar:
  order: 6
title: Starter Tutorial (Using Local LLMs)
---
...
Now we can create a tool for searching through documents using LlamaIndex. Rather than relying on any hosted-API default, we'll configure `Settings.embed_model` to `HuggingFaceEmbedding(model_name="BAAI/bge-base-en-v1.5")` so both indexing and retrieval use a local Sentence Transformers model with no API key required.
