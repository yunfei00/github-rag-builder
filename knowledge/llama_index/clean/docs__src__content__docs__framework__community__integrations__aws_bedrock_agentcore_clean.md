...
owner: run-llama
repo: llama_index
path: docs/src/content/docs/framework/community/integrations/aws_bedrock_agentcore.md
url: https://github.com/run-llama/llama_index/blob/main/docs/src/content/docs/framework/community/integrations/aws_bedrock_agentcore.md
---
---
title: Amazon Bedrock AgentCore Runtime and Tools
---

Amazon Bedrock AgentCore provides managed infrastructure for deploying and running production AI agents. The LlamaIndex integration lets you deploy agents to AgentCore Runtime with a single line of code, and gives your agents access to sandboxed browser automation, code execution, and persistent memory -- all running in secure AWS environments.
...
- IAM permissions for `bedrock-agentcore:*` actions (see the AgentCore documentation)
- Python 3.9+

## Runtime

The `AgentCoreRuntime` adapter deploys any LlamaIndex agent to Amazon Bedrock AgentCore Runtime -- a managed compute platform for AI agents. It wraps `BedrockAgentCoreApp` from the `bedrock-agentcore` SDK, providing the required `POST /invocations` and `GET /ping` endpoints with automatic SSE streaming support.
...
> **Note:** You must first create a memory resource in the AgentCore console or via the AWS SDK to obtain a `memory_id`. See the AgentCore documentation for setup instructions.
...
- Tools Package on PyPI
- Memory Package on PyPI
