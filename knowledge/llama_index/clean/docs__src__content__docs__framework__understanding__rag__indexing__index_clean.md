...
owner: run-llama
repo: llama_index
path: docs/src/content/docs/framework/understanding/rag/indexing/index.mdx
url: https://github.com/run-llama/llama_index/blob/main/docs/src/content/docs/framework/understanding/rag/indexing/index.mdx
---
---
title: Indexing
---
...
There are many types of embeddings, and they vary in efficiency, effectiveness and computational cost. If you don't configure one, `OpenAIEmbedding` falls back to `text-embedding-ada-002`. For new projects we recommend passing `OpenAIEmbedding(model="text-embedding-3-small")` explicitly, or using a local model like `HuggingFaceEmbedding("BAAI/bge-small-en-v1.5")` (see the embeddings guide). If you are using different LLMs you will often want to use different embeddings.
...
```
  ...
  `from_documents` also takes an optional argument `show_progress`. Set it to
