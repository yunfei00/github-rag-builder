...
There are [many types of embeddings](/python/framework/module_guides/models/embeddings), and they vary in efficiency, effectiveness and computational cost. If you don't configure one, `OpenAIEmbedding` falls back to `text-embedding-ada-002`. For new projects we recommend passing `OpenAIEmbedding(model="text-embedding-3-small")` explicitly, or using a local model like `HuggingFaceEmbedding("BAAI/bge-small-en-v1.5")` (see the [embeddings guide](/python/framework/module_guides/models/embeddings#choosing-an-embedding-model)). If you are using different LLMs you will often want to use different embeddings.
