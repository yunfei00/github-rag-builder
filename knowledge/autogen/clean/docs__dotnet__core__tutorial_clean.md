...
owner: microsoft
repo: autogen
path: docs/dotnet/core/tutorial.md
url: https://github.com/microsoft/autogen/blob/main/docs/dotnet/core/tutorial.md
---
# Tutorial

> [!TIP]
> If you'd prefer to just see the code the entire sample is available as a project here.
...
Learn more about what an Agent ID is here.

### Create a handler

Now, we want `Modifier` to receive `CountMessage` and produce `CountUpdate` after it modifies the count. To do this, we need to implement the `IHandle` interface:
...
Learn more about topics and subscriptions here.
