...
owner: firecrawl
repo: firecrawl
path: examples/blog-articles/amazon-price-tracking/notebook.md
url: https://github.com/firecrawl/firecrawl/blob/main/examples/blog-articles/amazon-price-tracking/notebook.md
---
---
...
- Firecrawl for AI-based scraping of e-commerce websites
...
With that said, head over to the Firecrawl website and sign up for a free account (the free plan will work fine). You will be given an API key, which you should copy.
...
However, the project is far from perfect. Since we took a top-down approach to building this app, our project code is scattered across multiple files and often doesn't follow programming best practices. For this reason, I've recreated the same project in a much more sophisticated manner with production-level features. This new version on GitHub implements proper database session management, faster operations and overall smoother user experience. Also, this version includes buttons for removing products from the database and visiting them through the app.

If you decide to stick with the basic version, you can find the full project code and notebook in the official Firecrawl GitHub repository's example projects. I also recommend that you deploy your Streamlit app to Streamlit Cloud so that you have a functional app accessible everywhere you go.
...
Some of these features are implemented in the advanced version of the project, so definitely check it out!

Here are some more guides from our blog if you are interested:

- How to Run Web Scrapers on Schedule
- More about using Firecrawl's `scrape_url` function
- Scraping entire websites with Firecrawl in a single command - the /crawl endpoint
