...
owner: firecrawl
repo: firecrawl
path: examples/blog-articles/deploying_web_scrapers/notebook.md
url: https://github.com/firecrawl/firecrawl/blob/main/examples/blog-articles/deploying_web_scrapers/notebook.md
---
---
...
Firecrawl offers several key advantages compared to traditional Python web scraping libraries:
...
As an example, we will build a simple scraper for ProductHunt. Specifically, we will scrape the "Yesterday's Top Products" list from the homepage:
...
These commands set up a working directory for the scraper, along with a virtual environment and the main script. The last part also saves your Firecrawl API key, which you can get through their free plan by signing up for an account.
...
The workflow automatically handles repository interaction, dependency management, and data updates while providing both scheduled and manual execution options. You can read the official GitHub guide on workflow file syntax to learn more.
...
Heroku is a Platform-as-a-Service (PaaS) that makes deploying applications straightforward, even for beginners. While it removed its generous free tier in 2022, its basic $5 "dyno" plan still has some free features we can take advantage of for the purposes of this tutorial.
...
After this step, if you visit dashboard.heroku.com, your app must be visible.
...
Heroku uses an add-on called "Scheduler" for running periodic tasks. Install it with:
...
To learn more about Heroku and how to run Python applications on its servers, please refer to their Python support documentation.

## Deploying Web Scrapers With PythonAnywhere

PythonAnywhere is a cloud-based Python development environment that offers an excellent platform for hosting web scrapers. It provides a free tier that includes:
...
First, create a free account at pythonanywhere.com. Once logged in, follow these steps:
...
1. Go to the "Tasks" tab in your PythonAnywhere dashboard accessible via 
...
- Comprehensive guide on Firecrawl's `scrape_url` method
- How to generate sitemaps in Python using Firecrawl
