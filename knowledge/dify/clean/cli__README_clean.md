...
repo: dify
path: cli/README.md
url: https://github.com/langgenius/dify/blob/main/cli/README.md
---
# difyctl
...
```sh
curl -fsSL https://raw.githubusercontent.com/langgenius/dify/main/cli/scripts/install-r2.sh | DIFYCTL_R2_BASE= sh
```
...
| `DIFYCTL_INSTALL_DIR`   | `$HOME/.local/bin` | Directory the binary is written to (`/difyctl`).               |
...
```sh
curl -fsSL https://raw.githubusercontent.com/langgenius/dify/main/cli/scripts/install-r2.sh | DIFYCTL_R2_BASE= DIFYCTL_COMMIT=ce4af86 sh
```

Windows: `$env:DIFYCTL_R2_BASE=''; irm https://raw.githubusercontent.com/langgenius/dify/main/cli/scripts/install-r2.ps1 | iex` (same env vars, e.g. `$env:DIFYCTL_COMMIT='ce4af86'`).
...
Run `difyctl  --help` for per-command reference.
...
- `difyctl skills install --yes` — write to every detected agent, printing each path. `--agent claude-code[,cursor]` restricts to a subset; `` forces one explicit directory (handy when your agent isn't detected).
...
| Knob                     | Effect                                         |
| ------------------------ | ---------------------------------------------- |
| `--http-retry `       | Per-invocation override. `0` disables retries. |
| `DIFYCTL_HTTP_RETRY=` | Process-level default.                         |
